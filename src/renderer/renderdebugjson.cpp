...
auto writeRenderDebugJson(const char* path, const RenderDebugSnapshot& s, const std::function<std::string(uint32_t)>& primPath) -> bool {
    ...
    std::fprintf(f, "  \"scene\": {\"instances\": %u, \"primFirstInstances\": %u, \"materials\": %u, \"materialsWithTexture\": %u, \"lights\": %u, \"hasSun\": %s, \"sunDirection\": [%.4f, %.4f, %.4f], \"sunRadiance\": [%.4f, %.4f, %.4f], \"shadowMap\": [%u, %u]},\n", s.instanceCount, s.primFirstInstances, s.materialCount, s.materialsWithTexture, s.lightCount, s.hasSun ? "true" : "false", s.sunDirection.x, s.sunDirection.y, s.sunDirection.z, s.sunRadiance.x, s.sunRadiance.y, s.sunRadiance.z, s.shadowMapExtent.width, s.shadowMapExtent.height);
